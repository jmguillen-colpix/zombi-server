...
const shutdown = (db_name) => {
    ...
    try {
        ...
        oracledb.getPool(db_name).close(1, (err) => {
            ...
            if (err) { log(err.message, "oracle/shutdown", true); }
            else { log("Oracle (" + config.db[db_name].host + ":" + config.db[db_name].port + "/" + config.db[db_name].name + ") disconnected", "oracle/shutdown"); }
