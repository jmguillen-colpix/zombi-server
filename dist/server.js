    ...
    if (config.server.log.access_enabled && access_file_desc) {
        try {
            ...
            fs.write(access_file_desc, buffer, 0, buffer.length, null, (err) => {
                if (err) {
                    log("Error writing access file", "server/access_write", true);
                }
            });
        }
        catch (error) {
            log(error.message, "server/access_write", true);
        }
    }
};
const execute = (mod, fun, args, token, seq, ip, ua) => __awaiter(this, void 0, void 0, function* () {
    try {
        ...
        if (mod === "sys_login" && fun === "login") {
            log(`Executing ${mod}/${fun} ("${args[0]}","*********","${args[2]}")`, "server/execute");
        }
        else {
            log(`Executing ${mod}/${fun} (${trimmed_args})`, "server/execute");
        }
        if (token) {
            session.update(token);
            if (yield session.check(token)) {
                if (yield security.authorize(token, mod)) {
                    return yield _execute(mod, fun, args, token, seq, ip, ua);
                }
                else {
...
const _execute = (mod, fun, args, token, seq, ip, ua) => __awaiter(this, void 0, void 0, function* () {
    try {
        ...
        if (typeof action[fun] === "function") {
            const [error, data, message] = yield action[fun](args, { token: token, sequence: seq });
