...
const log_file = (message, context, error = false) => {
    ...
    if (log_file_desc) {
        fs.write(log_file_desc, buffer, 0, buffer.length, null, (err) => {
            if (err) {
                log_console("Error writing log file", "server/log_file_write", true);
            }
        });
    }
};
const log = (message, context, error = false) => {
    if (message === "shutdown") {
        try {
            ...
            if (typeof context === "function") {
                context(error.message, false);
            }
        }
    }
    else {
        if (config.server.log.console_enabled) {
            log_console(message, context, error);
        }
        if (config.server.log.file_enabled && log_file_desc) {
            log_file(message, context, error);
        }
    }
};
module.exports = log;
