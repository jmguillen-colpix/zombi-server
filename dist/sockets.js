...
const heartbeat = () => {
    try {
        ...
        if (tokens.length === 0) {
            log("Nobody is connected", "sockets/heartbeat");
        }
...
const send_message_to_user = (user_id, context = "none", data = []) => __awaiter(this, void 0, void 0, function* () {
    try {
        const reply = yield db.sql(`select token from ${db.table_prefix()}sessions where user_id = :user_id`, [user_id]);
...
const send_message_broadcast = (context = "none", data = []) => __awaiter(this, void 0, void 0, function* () {
    try {
        const reply = yield db.sql("select token from zombi_sessions");
        if (reply.info.rows === 0) {
